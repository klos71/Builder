class Player {
	...
	update = () => {
		...
		if (
			this.x == this.home.getPos().x + 8 &&
			this.y == this.home.getPos().y + 8
		) {
			this.state = 'Home';
			this.energy += 0.2;
		}
		backGroundTiles.forEach(el => {
			if (this.x > el.getPos().x && this.x < el.getPos().x + 32) {
				if (this.y > el.getPos().y && this.y < el.getPos().y + 32) {
					if (el.getType() == 1) {
						this.speed += 0.25;
					} else {
						this.speed = 2;
	...
	work = () => {
		...
		try {
			var closest = this.getClosest(this.x, this.y, factorys);
			var workX = closest.x + 8;
			var workY = closest.y + 8;
			this.move(workX, workY);

			if (this.x > closest.getPos().x && this.x < closest.getPos().x + 32) {
				if (this.y > closest.getPos().y && this.y < closest.getPos().y + 32) {
					if (this.energy > 3) {
	...
	shop = () => {
		...
		try {
			var closest = this.getClosest(this.x, this.y, shops);
	...
	goHome = () => {
		...
		try {
			...
			if (this.x > this.home.getPos().x && this.x < this.home.getPos().x + 32) {
				if (
					this.y > this.home.getPos().y &&
					this.y < this.home.getPos().y + 32
				) {
					this.state = 'Home';
					this.energy += 0.2;
				}
			...
			if (this.x.toFixed(0) == homeX && this.y.toFixed(0) == homeY) {
			}
		} catch (err) {
	...
	getClosest = (startX, startY, array) => {
		var closest = this.getDistance({ x: startX, y: startY }, array[0].getPos());
		var fact = array[0];
		for (var i = 0; i < array.length; i++) {
			var temp = this.getDistance({ x: startX, y: startY }, array[i].getPos());
